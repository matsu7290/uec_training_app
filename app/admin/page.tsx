...
export default function AdminPage() {
  ...
  const handleUpdateProfile = async (id: string) => {
    const { error } = await supabase
      .from("profiles")
      .update({ role: editRole, grade: editGrade, is_admin: editIsAdmin })
      .eq("id", id);
  ...
  const handleAddAllowedMember = async (e: React.FormEvent) => {
    e.preventDefault();
    const studentId = newStudentId.trim().toLowerCase();
    const studentName = newMemberName.trim();
  ...
  const inputClass =
    "w-full bg-slate-100 dark:bg-slate-900 border-none rounded-xl p-3 text-sm font-bold focus:ring-2 focus:ring-blue-500 outline-none";

  return (
    <div className="max-w-2xl mx-auto p-4 mb-24">
      <h1 className="text-2xl font-black italic mb-6 tracking-tighter bg-gradient-to-r from-red-500 to-orange-500 bg-clip-text text-transparent uppercase">
        Admin Control
      </h1>
        ...
        <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4">
          ...
          <div className="grid gap-3">
            {filteredProfiles.map((p) => (
              <div
                key={p.id}
                className={`p-4 rounded-[2rem] border transition-all ${editingProfileId === p.id ? "bg-blue-50 border-blue-200 dark:bg-blue-900/20" : "bg-white dark:bg-slate-800 border-slate-100 dark:border-slate-700"}`}
              >
                ...
                {editingProfileId === p.id ? (
                  <div className="mt-4 p-4 bg-white dark:bg-slate-900 rounded-2xl space-y-4 shadow-inner">
                    ...
                    <div className="flex gap-2">
                      <button
                        ...
                        className="flex-1 bg-blue-600 text-white py-3 rounded-xl font-black text-xs"
                      >
                        保存する
                      </button>
                      <button
                        onClick={() => setEditingProfileId(null)}
                        className="px-4 bg-slate-200 dark:bg-slate-700 rounded-xl font-black text-xs"
                      >
                        戻る
                      </button>
                    </div>
                  ...
                  <div className="mt-3 flex gap-2">
                    <span className="text-[9px] font-black px-2 py-1 bg-slate-100 dark:bg-slate-700 rounded-md text-slate-500">
                      {p.grade || "学年未設定"}
                    </span>
                    <span
                      className={`text-[9px] font-black px-2 py-1 rounded-md ${p.role === "部長" ? "bg-amber-100 text-amber-600" : "bg-blue-50 text-blue-500"}`}
                    >
                      {p.role}
                    </span>
                    {p.is_admin && (
                      <span className="text-[9px] font-black px-2 py-1 bg-red-50 text-red-500 rounded-md">
                        ADMIN
                      </span>
                    )}
                  </div>
      ...
      {activeTab === "events" && (
        <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4">
          <form
            ...
            className="bg-white dark:bg-slate-800 p-6 rounded-[2rem] shadow-sm border border-slate-100 dark:border-slate-700 space-y-4"
          >
            <h2 className="font-black text-sm uppercase tracking-widest text-blue-500">
              {editingEventId ? "イベント編集" : "新規イベント作成"}
            </h2>
            <input
              value={eventTitle}
            ...
            <div className="flex gap-2">
              <button
                type="submit"
                className="flex-1 bg-slate-900 dark:bg-white dark:text-slate-900 text-white py-3 rounded-xl font-black text-xs uppercase"
              >
                {editingEventId ? "Update" : "公開する"}
              </button>
              {editingEventId && (
                <button
                  type="button"
                  onClick={() => setEditingEventId(null)}
                  className="px-6 bg-slate-100 dark:bg-slate-700 rounded-xl font-black text-xs"
                >
                  CANCEL
                </button>
              )}
          ...
          <div className="space-y-2">
            {events.map((e) => (
              <div
                key={e.id}
                className="flex justify-between items-center p-4 bg-white dark:bg-slate-800 rounded-2xl border border-slate-100 dark:border-slate-700"
              >
                <div>
      ...
      {activeTab === "whitelist" && (
        <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4">
          <form
            onSubmit={handleAddAllowedMember}
            className="bg-slate-900 text-white p-6 rounded-[2rem] space-y-4"
          >
            ...
            <input
              value={newStudentId}
              onChange={(e) => setNewStudentId(e.target.value.toLowerCase())}
              placeholder="学籍番号 (m2411728)"
              required
              className="w-full bg-slate-800 border-none rounded-xl p-3 text-xs font-bold text-white"
            />
            <input
              value={newMemberName}
            ...
            <button
              type="submit"
              className="w-full bg-white text-slate-900 py-3 rounded-xl font-black text-xs uppercase"
            >
              名簿に追加
            </button>
          </form>
          <div className="bg-white dark:bg-slate-800 rounded-[2rem] p-6 border border-slate-100 dark:border-slate-700">
            ...
            <div className="space-y-2 max-h-60 overflow-y-auto pr-2 custom-scrollbar">
              {allowedMembers.map((am) => (
                <div
                  key={am.student_id}
                  className="flex justify-between items-center py-2 border-b border-slate-50 dark:border-slate-700 text-[11px]"
                >
